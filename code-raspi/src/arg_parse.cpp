...
static void print_aliases(const std::list<std::string> aliases, std::string &str)
{
    str.clear();
    for (auto it = aliases.begin(); it != aliases.end(); ++it)
    {
        if (it != aliases.begin()) str.append(", ");
        str.append(it->c_str());
    }
}
...
void ArgumentParser::print_usage(FILE *file)
{
    ...
    for (auto it_arg = arguments.begin(); it_arg != arguments.end(); it_arg++)
    {
        print_aliases(it_arg->aliases, alias_str);
        if (alias_str.length() > max_alias_len) max_alias_len = alias_str.length();
    }

    for (auto it = arguments.begin(); it != arguments.end(); it++)
    {
        ArgumentDescription &arg = *it;
        print_aliases(arg.aliases, alias_str);
        while (alias_str.length() < max_alias_len + 2)
            alias_str.append(" ");
        fprintf(file, "%s%s\n", alias_str.c_str(), arg.description.c_str());
    }
    fprintf(file, "\n");
}
